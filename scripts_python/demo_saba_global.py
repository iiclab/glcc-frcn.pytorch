...
def parse_args():
  ...
  parser.add_argument('--load_dir', dest='load_dir',
                      ...
                      default="/home/usrs/nagayosi/Program_sababox/faster-rcnn.pytorch/models_global")
...
#      'Blue mackerel redline', 'Chub mackerel redline', 'Hybrid redline'])
  ...
  load_name = 'models_global/{}/saba_20171219_global_train/faster_rcnn_1_{}_833.pth'.format(args.net, args.epoch)
  result_dir = 'result_{}'.format(args.net)
  ...
  while (num_images > 0):
      ...
      #gt_l = np.zeros((5), dtype=np.float32)
      ...
      for j in xrange(1, 4):
          ...
          if inds.numel() > 0:
            ...
            all_dets = np.vstack((all_dets, cls_dets.cpu().numpy()))
            all_dets_cls += [pascal_classes[j] for _ in range(len(cls_dets))]
            all_dets_cls_ind += [j for _ in range(len(cls_dets))]
            #if vis:
            #    im2show = vis_detections(im2show, pascal_classes[j], cls_dets.cpu().numpy(), thresh=0.5)
            
      if len(all_dets) > 0:
        ind = np.argmax(all_dets[..., -1])
        dets_saba = all_dets[ind]
        dets_saba_cls = all_dets_cls[ind]
        dets_saba_cls_ind = all_dets_cls_ind[ind]
      ...
      if len(all_dets) > 0:
        ...
        iou_x1 = np.maximum(dets_saba[0], gt[0])
        iou_y1 = np.maximum(dets_saba[1], gt[1])
        iou_x2 = np.minimum(dets_saba[2], gt[2])
        iou_y2 = np.minimum(dets_saba[3], gt[3])
        iou_w = np.maximum(iou_x2 - iou_x1, 0)
        iou_h = np.maximum(iou_y2 - iou_y1, 0)
        iou_area= iou_w * iou_h
        dets_saba_area = np.maximum((dets_saba[2] - dets_saba[0]) * (dets_saba[3] - dets_saba[1]), 0)
        gt_area = np.maximum((gt[2] - gt[0]) * (gt[3] - gt[1]), 0)
        iou = iou_area / (dets_saba_area + gt_area - iou_area)

        if iou >= 0.5:
            N_det += 1.
            P_table[gt[-1], int(dets_saba_cls_ind)] += 1
            if int(dets_saba_cls_ind) == int(gt[-1]):
                #color = (0,0,255)
                color = 'red'
                #P_table[gt[-1], int(dets_saba_cls_ind)] += 1.
            else:
            ...
            P_table[gt[-1], 0] += 1.
        if vis:
            _d = tuple(int(np.round(x)) for x in dets_saba[:4])
            score = dets_saba[-1]
            ax.add_patch(
                plt.Rectangle((_d[0], _d[1]), _d[2]-_d[0], _d[3]-_d[1], fill=False, edgecolor=color, linewidth=1.))
            ax.text(_d[0], _d[1]-2, '{:s} {:.3f}'.format(dets_saba_cls, score), bbox=dict(facecolor=color, alpha=0.5), fontsize=14, color='white')
            #cv2.rectangle(im2show, det[:2], det[2:4], color, 2)
            #cv2.putText(im2show, "{} : {:.3f}".format(dets_saba_cls, score), (det[0], det[1]+25), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,200,255), thickness=2)
      else:
          P_table[gt[-1], 0] += 1.
         

      # Get Local region
      all_dets_l = np.ndarray((0, 5), dtype=np.float32)
      all_dets_l_cls = []
      all_dets_l_cls_ind = []
      ...
      for j in xrange(4, 7):
          ...
          if inds.numel() > 0:
            ...
            all_dets_l = np.vstack((all_dets_l, cls_dets.cpu().numpy()))
            all_dets_l_cls += [pascal_classes[j] for _ in range(len(cls_dets))]
            all_dets_l_cls_ind += [j for _ in range(len(cls_dets))]

      # choice max probability local region
      iou = 0
      if len(all_dets_l) > 0:
        ind = np.argmax(all_dets_l[..., -1])
        dets_l = all_dets_l[ind]
        dets_l_cls = all_dets_l_cls[ind]
        dets_l_cls_ind = all_dets_l_cls_ind[ind]
        # evaluation between detection and gt 
        iou_x1 = np.maximum(dets_l[0], gt_l[0])
        iou_y1 = np.maximum(dets_l[1], gt_l[1])
        iou_x2 = np.minimum(dets_l[2], gt_l[2])
        iou_y2 = np.minimum(dets_l[3], gt_l[3])
        iou_w = np.maximum(iou_x2 - iou_x1, 0)
        iou_h = np.maximum(iou_y2 - iou_y1, 0)
        iou_area= iou_w * iou_h
        dets_l_area = np.maximum((dets_l[2] - dets_l[0]) * (dets_l[3] - dets_l[1]), 0)
        gt_area = np.maximum((gt_l[2] - gt_l[0]) * (gt_l[3] - gt_l[1]), 0)
        iou = iou_area / (dets_l_area + gt_area - iou_area)

        if iou >= 0.5:
            N_det_l += 1.
            if int(dets_l_cls_ind) == int(gt_l[-1]):
                color = (0,0,255)
                P_table_l[gt_l[-1]-3, int(dets_l_cls_ind)-3] += 1.
            else:
                color = (255,0,0)
        ...
        if vis:
            det = tuple(int(np.round(x)) for x in dets_l[:4])
            score = dets_l[-1]
            cv2.rectangle(im2show, det[:2], det[2:4], color, 2)
            cv2.putText(im2show, "{} : {:.3f}".format(dets_l_cls, score), (det[0], det[1]+25), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,200,255), thickness=2)
      else:
          P_table_l[gt_l[-1]-3, 0] += 1.
        """     
      #if vis:
  ...
  print("average process time >> {} ({}sec / {}images)".format(sum_time / N_all, sum_time, N_all))
