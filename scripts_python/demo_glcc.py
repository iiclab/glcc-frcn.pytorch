...
from model.faster_rcnn.glcc import GLCC
...
if __name__ == '__main__':
  ...
  frcn_load_name = 'models/{}_glcc/saba_20171219_train/glcc_faster_rcnn_1_{}_833.pth'.format(args.net, args.epoch)
  glcc_load_name = 'models/{}_glcc/saba_20171219_train/glcc_1_{}_833.pth'.format(args.net, args.epoch)
  result_dir = 'result_{}_glcc'.format(args.net)
  ...
  glcc = GLCC(fasterRCNN, use_cuda=True, classes=pascal_classes, net=args.net, pretrained=False, class_agnostic=args.class_agnostic)
  glcc.load_state_dict(torch.load(glcc_load_name)['model'])
  #params = {}
  #for k, v in torch.load('models/res101/saba_20171219_train/glcc_1_50_833.pth')['model'].items():
  #    if 'FRCN' not in k:
  #        params[k] = v
  ...
  print("load frcn >> {}".format(frcn_load_name))
  print("load glcc >> {}".format(glcc_load_name))
  ...
  while (num_images > 0):
      ...
      rois_label, glcc_prob, glcc_loss = glcc(im_data, im_info, gt_boxes, num_boxes)

      glcc_prob = F.softmax(glcc_prob, dim=1)
      ...
      for j in xrange(1, 4):
          ...
          if inds.numel() > 0:
            ...
            det_g = np.r_[cls_dets[np.argmax(cls_dets[..., -1])], [j]]
          ...
          # get local region
          inds = torch.nonzero(scores[:,j+3]>thresh).view(-1)
          if inds.numel() > 0:
            cls_scores = scores[:,j][inds]
            _, order = torch.sort(cls_scores, 0, True)
            if args.class_agnostic:
              ...
              cls_boxes = pred_boxes[inds][:, (j+3)*4:(j+4)*4]
            cls_dets = torch.cat((cls_boxes, cls_scores.unsqueeze(1)), 1)
            det_l = np.r_[cls_dets[np.argmax(cls_dets[...,-1])], [j+3]]
      ...
      if len(region_g) > 0:
        ...
        if vis:
            ...
            ax.text(_d[0], _d[1] - 2,
                    '{:s} {:.3f}'.format(pascal_classes[int(pred_ind)], score),
