...
if __name__ == '__main__':
  ...
  pascal_classes = np.asarray(['__background__', 'mackerel', 'red line'])

  # initilize the network here.
  if args.net == 'vgg16':
    fasterRCNN = vgg16(pascal_classes, pretrained=False, class_agnostic=args.class_agnostic)
    load_name = 'models_2class/vgg16/saba_20171219_2class_train/faster_rcnn_1_100_833.pth'
    result_dir = 'result_vgg16_2class'
  ...
  while (num_images > 0):
      ...
      if len(all_dets) > 0:
        ...
        if vis:
            ...
            ax.text(_d[0], _d[1]-2,
                    '{:s} {:.3f}'.format(pascal_classes[1], score),
      ...
      if len(all_dets_l) > 0:
        ...
        if vis:
            _d = tuple(int(np.round(x)) for x in dets_l[:4])
            score = dets_l[-1]
            ax.add_patch(
                plt.Rectangle((_d[0], _d[1]), _d[2]-_d[0], _d[3]-_d[1],
                              fill=False, edgecolor=color, linewidth=1.))
            ax.text(_d[0], _d[3]-2,
                    '{:s} {:.3f}'.format(pascal_classes[2], score),
                    bbox=dict(facecolor=color, alpha=0.5),
                    fontsize=14, color='white')
            #cv2.rectangle(im2show, det[:2], det[2:4], color, 2)
